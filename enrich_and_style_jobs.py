...
# Job Search Keywords
JOB_KEYWORDS = ["busqueda", "búsqueda", "vacante", "buscamos", "oferta", "puesto", "contratando", "postular", "postulate", "postulá", "selección", "seleccion", "unete", "únete", "cv", "empleo", "trabaja", "rrhh"]
PROFILE_KEYWORDS = ["analista", "contable", "administra", "costo", "presupuesto", "finanza", "reporting", "auxiliar", "asistente", "junior", "estudiante", "operaciones"]
...
def process_single_company(item, relevance, sector):
    ...
    if website:
        ...
        if job_info["job_posting"]:
            obs.append(f"VACANTE DETECTADA: {job_info['job_posting']}")
...
def style_excel(file_path):
    ...
    # Soft Yellow/Orange (Vacante activa detectada)
    ...
    # ws.row_dimensions[1].height = 28 # Set header height
    ...
    for row in range(2, ws.max_row + 1):
        ...
        has_active_job = ("VACANTE DETECTADA" in str(obs_cell.value))
    ...
    for col in ws.columns:
        ...
        for cell in col:
            ...
            # If it's a URL, limit its weight on column width calculations
